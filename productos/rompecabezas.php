...
<body style="background-color: #e0e0df;">
    ...
    ?>
    <br><br><br><br>
    <h1 class="text-center">Productos en venta: Rompecabezas</h1>
    <?php
    //Paginador
    $limite = 4;
    $pagina = '';
    ...
    $resultado = $conexion->query("SELECT * FROM productos WHERE Categoria = 'Rompecabezas' LIMIT $inicio,$limite") or die($conexion->error);
    ?>
    ...
    <?php
    //Paginador
    $pagina_consulta = "SELECT * FROM productos WHERE Categoria = 'Rompecabezas'";
        ...
        <ul class="pagination justify-content-center">
            <?php if ($pagina > 1) { ?>
                <li class="page-item <?php echo $_GET['pagina'] <= 0 ? 'disabled' : '' ?> ">
                    <?php echo "<a class='page-link' href='rompecabezas.php?pagina=" . ($pagina - 1) . "'> Anterior </a>"; ?>
                </li>
            <?php
            }
            for ($i = 1; $i <= $fin; $i++) { ?>
                <li class="page-item <?php echo $_GET['pagina'] == $i ? 'active' : '' ?>">
                    <?php echo "<a class='page-link' href='rompecabezas.php?pagina=" . $i . "'>" . $i . "</a>"; ?>
                </li>
            <?php
            }
            if ($pagina <= $fin - 1) { ?>
                <li class="page-item">
                    <?php echo "<a class='page-link' href='rompecabezas.php?pagina=" . ($pagina + 1) . "'> Siguiente</a>"; ?>
                </li>
            <?php } ?>
        </ul>
    </nav>
